<template>
	...
	<div :class="[is_hidden ? 'rmc_row_hidden' : 'rmc_row']">
		...
		<div class="rmc_container rmc_inter_container">
			<select class="rmc_inter" v-if="inc>0" v-model="interValue" :name="interName" >
		...
		<div class="rmc_container rmc_criteria_container">
				<select name="search[]" class="rmc_criteria" v-model="dataFieldId">
		...
		<input type="button" v-if="isLast" class="bouton_add_criterie_rmc bouton" @click.prevent="$emit('addCriteria')" value="+">
	</div>
		<template v-else>
