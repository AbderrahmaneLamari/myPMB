...
if ($log_ok) {
	...
	if (! $dest) {
		...
		if ($allow_pnb) {
			$empr_onglet_menu .= "<li " . (($tab == "pnb") ? "class=\"subTabCurrent\"" : "") . "><a href='./empr.php?tab=pnb&lvl=pnb_devices'>" . htmlentities($msg['empr_menu_pnb'], ENT_QUOTES, $charset) . "</a></li>";
		}
	...
	switch ($lvl) {
		...
		case "scan_request" :
			...
			if ($allow_scan_request) {
    			print "<h3><span>" . $msg['empr_menu_scan_requests'] . "</span></h3>";
