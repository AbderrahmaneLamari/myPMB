...
// +-------------------------------------------------+
// $Id: shorturl_type.class.php,v 1.13 2021/04/13 14:34:26 dgoron Exp $
...
class shorturl_type {
	...
	public function get_display_shorturl_in_result($action = '', $type_search='') {
		...
		$html = '';
		if ($rss) {
		    $html.= "<span class=\"espaceResultSearch\">&nbsp;&nbsp;</span><span class=\"short_url\"><a target='_blank' href='".$this->get_shorturl('rss'). ($type_search ? "&type_search=".$type_search : '') ."' title='".$msg["short_url_generate"]."'>".$msg["short_url_generate"]."</a></span>";
		}
		if ($permalink) {
			$html.= "
