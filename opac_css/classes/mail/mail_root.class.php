...
// +-------------------------------------------------+
// $Id: mail_root.class.php,v 1.6.2.5 2023/11/14 17:05:17 dgoron Exp $
...
abstract class mail_root {
	...
	protected function get_mail_from_name() {
		...
		global $empr_location, $empr_prenom, $empr_nom;
		...
		switch ($this->get_setting_value('sender')) {
			...
			case 'docs_location':
				if(!empty($this->mail_from_id)) {
					...
					$docs_location = new docs_location($empr_location);
					return $docs_location->libelle;
				}
			case 'reader':
	...
	protected function get_mail_from_mail() {
		...
		global $empr_location, $empr_mail;
