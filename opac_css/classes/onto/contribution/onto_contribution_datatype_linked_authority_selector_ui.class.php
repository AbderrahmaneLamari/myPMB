...
// +-------------------------------------------------+
// $Id: onto_contribution_datatype_linked_authority_selector_ui.class.php,v 1.6.2.5 2021/08/31 14:17:26 qvarin Exp $
...
// 				$first = false;
	...
	private static function parse_file() {
	    ...
	    if(!$dejaParse){
	        ...
	        if ($key_file) {
	            $key_file_content=$KEY_CACHE_FILE_XML.md5(serialize(array(self::$aut_link_xml)));
	            $cache_php->setInCache($key_file_content, array(self::$aut_link_xml));
	            $cache_php->setInCache($key_file,$key_file_content);
	        }else{
	            $tmp = fopen($tempFile, "wb");
	            fwrite($tmp,serialize(array(self::$aut_link_xml)));
	            fclose($tmp);
	        }
	    }
	}
	...
	private static function generate_relation_type_authority($property, $selected) {
	    ...
	    foreach($marc_list->table as $value => $label){
	        // 	        if ($i == 0 && is_null($selected)) {
	        // 	            $selected = $value;
	        // 	        }
	        $display_none = "";
	        if (count($list_values) && !in_array($value, $list_values, true)) {
	            $display_none = 'style="display:none;"';
	        } else {
	            if (is_null($selected)) {
	                $selected = $value;
