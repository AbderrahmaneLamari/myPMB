...
// +-------------------------------------------------+
// $Id: onto_contribution_datatype_thumbnail.class.php,v 1.10.4.1 2023/12/28 14:52:03 gneveu Exp $
...
class onto_contribution_datatype_thumbnail extends onto_common_datatype_file
{
    ...
    {
        if (is_string($this->value) && (strlen($this->value) < 131072)) {
            return true;
        }
        return false;
    }
    ...
    public static function get_values_from_form($instance_name, $property, $uri_item)
    {
        ...
        if(count($file)) {
            ...
            ${$var_name} = $values;
        } elseif (${$var_name}[0]['value'] == "" && !empty($_POST[$var_name][0]['data'])) {
            $values[] = array(
    ...
    public function get_value($all = false)
    {
        ...
        if ($all && $decoded) {
            if (is_object($decoded) && isset($decoded->id_upload_directory, $decoded->path)) {
