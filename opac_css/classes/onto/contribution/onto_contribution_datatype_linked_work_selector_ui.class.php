...
// +-------------------------------------------------+
// $Id: onto_contribution_datatype_linked_work_selector_ui.class.php,v 1.17.2.2 2021/09/09 14:18:26 tsamson Exp $
...
// 		$selector_url = $this->get_resource_selector_url($property->range[0]);
		...
		if(sizeof($datas)){
			...
			foreach($datas as $key=>$data){
				...
				$selector = static::get_selector($property->pmb_name, '!!onto_row_id!![!!onto_row_order!!][relation_type_work]', (isset($formated_value['relation_type_work']) ? $formated_value['relation_type_work'] : ""));
...
// 				}
				...
				if ($property->has_linked_form && $first) {
				    ...
				    if ($access_granted) {
				        ...
				        if ($linked_forms && !empty($formated_value['work']) && !empty($formated_value['work']['value'])) {
			...
			$selector = static::get_selector($property->pmb_name, '!!onto_row_id!![!!onto_row_order!!][relation_type_work]', "");
			$inside_row = str_replace(array(
			    '!!onto_row_content_linked_record_selector!!',
			    "!!onto_row_content_marclist_range!!",
			    "!!onto_current_element!!",
	...
	protected static function get_selector($pmb_name, $name, $selected) {
	    global $charset,$msg;
