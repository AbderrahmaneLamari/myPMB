...
class onto_common_item {
	...
	/**
	 * identifiant de l'item dans la table onto_uri 
	 * @var int
	 */
	protected $id;
		...
		if(sizeof($this->onto_class->get_properties())){
			...
			foreach($this->onto_class->get_properties() as $uri_property){
				...
				if(!$flag || (in_array($flag,$property->flags))){
					...
					$movable_div = str_replace('!!data_pmb_uniqueid!!', $property->pmb_name, $movable_div);
		...
		if(!class_exists($class_name)){
			...
			if(!class_exists($class_name)){
				...
				if(!class_exists($class_name)){
					...
					if(!class_exists($class_name)){
						$class_name = "onto_".$onto_name."_datatype_".$pmb_datatype.$suffix;
						if(!class_exists($class_name)){
							//loupé, heu là on regarde dans le common...
							if($onto_name == "common"){
