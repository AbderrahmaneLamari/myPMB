...
// +-------------------------------------------------+
// $Id: list_ui.class.php,v 1.1.2.12 2024/01/03 14:50:34 dgoron Exp $
...
// 		$this->set_settings_in_session();
	...
	protected function get_search_filter_simple_selection($query, $name='', $message_all='', $options=array()) {
		...
		switch ($this->get_selected_setting_filter($name, 'selection_type')) {
			...
			case 'selector' :
			default :
				$selector .= "<select id='".$this->objects_type."_".$name."' name='".$this->objects_type."_".$name."' class='list_ui_simple_selector ".$this->objects_type."_simple_selector'>";
				if($message_all) {
	...
	protected function get_search_filter_multiple_selection($query, $name='', $message_all='', $options=array()) {
		...
		switch ($this->get_selected_setting_filter($name, 'selection_type')) {
		    ...
		    case 'selector':
		    default :
		    	$selector .= "<select id='".$this->objects_type."_".$name."' name='".$this->objects_type."_".$name."[]' multiple='3' class='list_ui_multiple_selector ".$this->objects_type."_multiple_selector'>";
		    	if($message_all) {
			...
			case 'expandable_table':
				foreach($grouped_objects as $group_label=>$objects) {
					...
					if(empty($objects[0])) {
						...
						$display .= $this->gen_plus($uid_group,$this->get_display_group_label($group_label, count($objects)),$display_expandable_content,$expanded_display);
					} else {
			...
			case 'edit':
				$display .= "
					<span class='list_ui_selection_action_configuration_columns_".$name." ".$this->objects_type."_selection_action_configuration_columns_".$name."'>
						<label>".htmlentities($label, ENT_QUOTES, $charset)."</label>
					...
					<span class='list_ui_selection_action_configuration_button_".$name." ".$this->objects_type."_selection_action_configuration_button_".$name."' id='".$this->objects_type."_selection_action_configuration_button_".$name."' style='display:none;'>
