...
// +-------------------------------------------------+
// $Id: cms_module_bannetteslistabon_view_bannetteslistabon.class.php,v 1.5 2021/09/30 10:31:05 btafforeau Exp $
...
function valid_form(choice){
	...
	if(choice=='abonn'){
		if(document.getElementsByName('bannette_filtered_list[]').length) {
...
<form action='' method='post' name='bannette_subscription'>
	...
	<div class='itemAlerte'>
		...
		<input type='hidden' name='bannette_abon[{{bannette.id}}]' value='1' />
        {% else %}
		...
		{% endif %}
        <input type='hidden' name='bannette_filtered_list[]' value='{{bannette.id}}' />
