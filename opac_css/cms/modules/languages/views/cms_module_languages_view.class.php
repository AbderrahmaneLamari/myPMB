...
// +-------------------------------------------------+
// $Id: cms_module_languages_view.class.php,v 1.1.2.4 2022/01/07 09:05:15 moble Exp $
...
class cms_module_languages_view extends cms_module_common_view{
	...
	public function generate_list($datas) {
        ...
        $form .= "<form id='change_lang_form' method=\"post\" action=\"".$action."\" >
                  <input type='hidden' id='change_lang_input' value=''  name=\"lang_sel\"/>";
	    ...
	    if(count($_POST)) {
	        foreach ($_POST as $name=>$value) {
	            if(is_string($value) && $name !="lang_sel") {
	                $form .= "<input type='hidden' name=\"".$name."\" value=\"".htmlentities($value, ENT_QUOTES, $charset)."\" />";
	            } elseif(is_array($value)) {
	                foreach ($value as $sub_key=>$sub_value) {
	                    if(is_string($sub_value)) {
	                        $form .= "<input type='hidden' name=\"".$name."[".$sub_key."]\" value=\"".htmlentities($sub_value, ENT_QUOTES, $charset)."\" />";
	                    }
	                }
	            }
	        }
