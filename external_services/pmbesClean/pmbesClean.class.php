...
// +-------------------------------------------------+
// $Id: pmbesClean.class.php,v 1.34.2.1 2021/12/13 15:31:05 dgoron Exp $
...
class pmbesClean extends external_services_api_class {
	...
	public function indexGlobal() {
		...
		if (SESSrights & ADMINISTRATION_AUTH) {
			...
			pmb_mysql_query("delete from notices_fields_global_index");
	...
	public function genSignatureDocnum() {
		...
		$result = '';
		if (SESSrights & ADMINISTRATION_AUTH) {
			$result .= "<h3>".htmlentities($msg["gen_signature_docnum"], ENT_QUOTES, $charset)."</h3>";
			...
			$result .= $count." ".htmlentities($msg["gen_signature_docnum_status_end"], ENT_QUOTES, $charset);
	...
	public function cleanConcept() {
		global $msg, $charset, $base_path, $PMBusername;
		...
		if (SESSrights & ADMINISTRATION_AUTH) {
			...
			$query = pmb_mysql_query("SELECT count(distinct id_item) FROM skos_words_global_index");
	...
	public function indexAuthorities() {
		...
		if (SESSrights & ADMINISTRATION_AUTH) {
			...
			//remise a zero de la table au début
			pmb_mysql_query("truncate authorities_words_global_index");
			pmb_mysql_query("truncate authorities_fields_global_index");
			pmb_mysql_query("ALTER TABLE authorities_words_global_index DISABLE KEYS");
			pmb_mysql_query("ALTER TABLE authorities_fields_global_index DISABLE KEYS");
			...
			pmb_mysql_query("ALTER TABLE authorities_words_global_index ENABLE KEYS");
			pmb_mysql_query("ALTER TABLE authorities_fields_global_index ENABLE KEYS");
		} else {
			$result .= sprintf($msg["planificateur_rights_bad_user_rights"], $PMBusername);
		}
