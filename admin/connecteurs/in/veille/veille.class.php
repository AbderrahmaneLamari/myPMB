...
// +-------------------------------------------------+
// $Id: veille.class.php,v 1.10.2.3 2021/10/26 10:56:04 gneveu Exp $
...
class veille extends connector {
    ...
    public function make_serialized_source_properties($source_id) {
        ...
        $this->sources[$source_id]["PARAMETERS"]=serialize(['sources_watches'=>$sources_watches,'items_interesting'=>$items_interesting]);
    }
    ...
    public function search($source_id,$search_query,$search_id) {
        ...
        foreach($search_query as $amterm) {
            switch ($amterm->ufield) {
                case '330':
                    $query .= " AND item_summary LIKE '%". rawurlencode($amterm->values[0]) ."%'";
                    $query .= " OR item_content LIKE '%". rawurlencode($amterm->values[0]) ."%'";
                    break;
                case '896':
                    $query .= " AND item_logo_url LIKE '%". rawurlencode($amterm->values[0]) ."%'";
                    break;
                case '214$d':
                case '210$d':
                case '210$d214$d':
                    $query .= " AND item_publication_date LIKE '%". rawurlencode($amterm->values[0]) ."%'";
                    break;
                case '200$a':
                    $query .= " AND item_title LIKE '%". rawurlencode($amterm->values[0]) ."%'";
                    break;
                case '610':
                    $ids = array();
                    $ids_item = array();
                    ...
                    if (count($ids_item)){
                        $query .= " AND id_item IN (".implode(',',$ids_item).") ";
                    }
                    break;
                case '60X':
                    $ids_categ = array();
                    ...
                    if (!count($ids_to_save)) break;
                    
                    $query .= " AND id_item IN (".implode(',',$ids_to_save).") ";
                    
                    break;
                case 'XXX':
                    if ($amterm->values[0] == "*") break;
                default:
                    $query .= " AND item_index_sew LIKE '%". rawurlencode($amterm->values[0]) ."%'";
                    break;
            }
        }
