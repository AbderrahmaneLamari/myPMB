...
// +-------------------------------------------------+
// $Id: RegistrationModel.php,v 1.65.2.5 2023/08/03 09:49:08 gneveu Exp $
...
class RegistrationModel extends Model
{
    ...
    public static function deleteRegistration(int $id)
    {
        ...
        if (! empty($maillingTypeOrm) && ! empty($maillingTypeOrm[0])) {
            ...
            MailingAnimationModel::sendMail([
                $registrationModel
            ], $registrationModel->numAnimation, $template, $temp, $maillingTypeOrm[0]->num_sender);
        }
    }
    ...
    public static function addRegistration(object $data)
    {
        ...
        foreach ($data->animationsSelected as $idAnimation) {
            ...
            if (! empty($maillingTypeOrm) && ! empty($maillingTypeOrm[0])) {
                ...
                MailingAnimationModel::sendMail([
                    $registrationModel
                ], $idAnimation, $template, $temp, $maillingTypeOrm[0]->num_sender);
            }
        }
    ...
    {
        global $opac_animations_only_empr;
    ...
    public static function validateRegistration(int $id)
    {
        ...
        if (! empty($maillingTypeOrm) && ! empty($maillingTypeOrm[0])) {
            ...
            MailingAnimationModel::sendMail([
                $registration
            ], $registration->numAnimation, $template, $temp, $maillingTypeOrm[0]->num_sender);
        }
    ...
    public static function reviewRegistration(RegistrationOrm $registration, int $freeQuotas)
    {
        ...
        if (! empty($maillingTypeOrm) && ! empty($maillingTypeOrm[0])) {
            ...
            MailingAnimationModel::sendMail([
                new RegistrationModel($registrationOrm->{RegistrationOrm::$idTableName})
            ], $registration->num_animation, $template, $campaignDatas, $maillingTypeOrm[0]->num_sender);
        }
    ...
    public static function registrationAccount(object $registration, $registrationList, int $statusRegistration, int $sens)
    {
        ...
        if (0 != $totalToPay && ! empty($registration->numEmpr) && self::VALIDATED == $registration->numRegistrationStatus) {
            ...
            $comment =
            $titre . " " .
            GlobalContext::msg("animation_comment_cmpte_animation_of") . " " .
            $date->format("d/m/Y") . " <a href='../animations.php?categ=animations&action=view&id=" . $animation->id . "' target='_blank'>" . $animation->name . " </a>";
