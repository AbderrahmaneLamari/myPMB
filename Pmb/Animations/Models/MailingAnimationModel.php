...
class MailingAnimationModel extends MailingModel
{
    ...
    public static function computeMail($idAnim, $mailing_list_choice = [])
    {
        ...
        foreach ($anim->mailings as $mailing) {
            ...
            if ($mailing->mailingsType->periodicity == MailingTypeModel::MAILING_REGISTRATION || 
                $mailing->mailingsType->periodicity == MailingTypeModel::MAILING_CONFIRMATION ||
                $mailing->mailingsType->periodicity == MailingTypeModel::MAILING_ANNULATION
            ){
                continue;
            }
