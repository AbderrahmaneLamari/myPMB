...
// +-------------------------------------------------+
// $Id: Portal.php,v 1.9 2022/07/19 08:59:28 qvarin Exp $
...
class Portal
{
    ...
    private static $type_page_opac = null;

    private const IN_POST_AND_GET = 0;

    private const IN_GET = 1;

    private const IN_POST = 2;
    ...
    public const SUB_PAGES = array(
        ...
        "search_result_simple_search" => 201,
    ...
    private static function getValue($index, $where = self::IN_POST_AND_GET)
    {
            ...
            case 'cmspage':
                ...
                if (strpos($url, 'pageid')) {
                    ...
                    if (strpos(substr($url, strpos($url, 'pageid') + 7), '&')) {
                        return "25" . str_pad(substr($url, strpos($url, 'pageid') + 7, strpos(substr($url, strpos($url, 'pageid') + 7), '&')), 2, "0", STR_PAD_LEFT);
                    } else {
                        return "25" . str_pad(substr($url, strpos($url, 'pageid') + 7), 2, "0", STR_PAD_LEFT);
                    }
                } else {
    ...
    public static function getLabel($type)
    {
        ...
        return static::$type_page_opac[$type] ?? "";
    }
