...
// +-------------------------------------------------+
// $Id: RootView.php,v 1.21.2.29 2023/12/08 09:30:02 jparis Exp $
...
class RootView extends Root
{
     ...
     * @param int $limit
     */
    protected function limitData(array &$data, int $limit)
    {
        if ($limit == 0) {
            return;
        }
    ...
    protected function filterData(array &$data, int $entityId)
    {
    ...
    public function duplicate($param = null)
    {
        ...
        if ($newEntity->id != 0) {
            if (empty($this->settings->locked)) {
                ...
                foreach ($this->childs as $child) {
                    $newEntityChild = $child->duplicate($newEntity);
     ...
     */
    protected function updateViewChild($oldIdChild = 0, $newIdChild = 0)
    {}
