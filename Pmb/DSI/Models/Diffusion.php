...
// +-------------------------------------------------+
// $Id: Diffusion.php,v 1.18.2.53 2023/11/29 13:55:28 rtigero Exp $
...
class Diffusion extends Root implements CRUD
{
    ...
    public function duplicate($param = null)
    {
        ...
        if ($this->numView != 0 && in_array("view", $param)) {
            ...
            if($this->view instanceof WYSIWYGView) {
	...
	public function fetchLastDiffusion()
    {
        ...
        if (empty($diffusionsHistory)) {
			...
			$diffusionHistory = DiffusionHistory::getInstance($diffusionsHistory[0]->id_diffusion_history, $this);
			$this->lastDiffusion = $diffusionHistory->getFormatedDate();
		}
        return true;
    }
    ...
    protected function updatedDiffusionDescriptors()
    {
        ...
        $this->diffusionDescriptors = [];
        foreach ($descriptors as $order => $descriptor) {
