...
// +-------------------------------------------------+
// $Id: consolidation.inc.php,v 1.57.2.1 2021/09/29 09:13:17 dgoron Exp $
...
function aff_type_page($param, $parser){
	...
	$page = array("recherche" => 1, "result" => 2, "result_noti" => 3, "result_aut" => 4, "aut" => 5, 
	            ...
	            "collstate_bulletins_display" => 32, "pixel" => 33, "animation" => 39
	);
...
function aff_sous_type_page($param,$parser){
	...
	if (!empty($post['onglet_persopac'])){
		$perso = $post['onglet_persopac'];
	} elseif (!empty($get['onglet_persopac'])){
	...
	//facettes
	if(isset($get['reinit_facette']) || isset($get['param_delete_facette'])) { //param_delete_facette peut être égal à 0
		return '308';
	}elseif(isset($get['facette_test'])){
		return '307';
	}
	...
	switch($niveau){		
		...
		case 'etageres_see':
			...
			break;
		case 'show_cart':
			if (isset($get['raz_cart'])) {
				$search_type = '805';
			} elseif (!empty($get['action']) && $get['action']=='del') {
				$search_type = '806';
