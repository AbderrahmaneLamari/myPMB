...
// +-------------------------------------------------+
// $Id: notice_tpl.inc.php,v 1.176.2.7 2023/12/22 09:19:49 qvarin Exp $
...
function aff_get_notice($param) {
	...
	if(!empty($notice['notice_info'])) {
		...
		$memo_notice->caddies=array();
		$caddies = caddie::get_cart_list_from_item('NOTI', 0, $memo_notice->notice_id);
		if(sizeof($caddies)) {
			foreach ($caddies as $caddie) {
				$memo_notice->caddies[]= $caddie['idcaddie'];
			}
		}
...
function gere_expl_bulletin($bulletin_id){
	...
	while (($ex = pmb_mysql_fetch_object($resultat))) {
		...
		$ex->caddies=array();
		$caddies = caddie::get_cart_list_from_item('EXPL', 0, $ex->expl_id);
		if(sizeof($caddies)) {
		    foreach ($caddies as $caddie) {
		        $ex->caddies[]= $caddie['idcaddie'];
		    }
		}
 ...
 * $param[4] : unique (0 = tous, 1 = dédoublonnage)
 */
...
function group($param){
	...
	if (empty($display_number)) {
		$display_number = count($array_id);
	}
	if ($display_number > count($array_id)) {
		$display_number = count($array_id);
	}
	if (empty($separator)) {
		$separator = ' - ';
...
/*
 * @param[0] = Tableau d'identifiants de notices ou identifiants séparés par une virgule
 * @param[1] = Nom de la propriété pour le filtrage
