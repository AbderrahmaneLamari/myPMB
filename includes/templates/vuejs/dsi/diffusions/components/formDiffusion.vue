<template>
	<div class="dsi-form-diffusion-container">
		<p class="diffusion-last-send">{{ messages.get('dsi', 'diffusion_last_send').replace('%s', lastDiffusionHistoryDate) }}</p>
		...
		<form action="#" method="POST" @submit.prevent="submit" class="dsi-form-diffusion">
			...
			<div class='row'>
				...
				<div class="left">
					...
					<input v-if="canManuallySend" type="button" class="bouton" :value="messages.get('dsi', 'dsi_send_diffusion_manually')" @click="sendDiffusionManually">			
				</div>
...
export default {
	...
	computed : {
        lastDiffusionHistoryDate: function() {
			...
			if(!  lastHistorySend || ! lastHistorySend.length) {
				return this.messages.get('dsi', 'diffusion_never_send');
			}
	...
	methods: {
		...
		sendDiffusionManually : async function() {
			if (this.canManuallySend && confirm(this.messages.get('dsi', 'dsi_send_diffusion_manually_alert'))) {
				let result = await this.ws.get(this.$root.categ, this.diffusion.id + "/send");
				if(! result.error) {
