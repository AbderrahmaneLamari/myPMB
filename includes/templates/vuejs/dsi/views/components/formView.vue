					...
					<div class="dsi-form-group">
						...
						<div class="dsi-form-group-content view-form-selected-type">
							...
							<button
								...
								@click.prevent="$root.$emit('openViewConfiguration', 2);">
			...
			<div class='row dsi-form-action'>
				<div class="left">
					<template v-if="is_model">
						...
						<input name="submit_model" type="submit" class="bouton" :value="messages.get('dsi', 'submit_model')">
					</template>
	...
	export default {
		...
		methods: {
			...
			submit: async function(e="") {
				...
				if (
					e && e.submitter &&
					(e.submitter.name === "submit_model" || e.submitter.name === "submit_model_from_modal") ||
					e.name === "submit_model_from_modal"
				) {
					lastId = this.view.id;
				...
				} else if (this.view.model && this.view.id == 0) {
