	...
	export default {
		...
		methods: {
			getSelectorList: async function() {
				let response = await this.ws.get('items', 'getSelectorList/' + encodeURI(this.child));
				if (response.error) {
            ...
            getHaveSubSelector: async function() {
                if(this.selector.namespace !== "") {
                    let response = await this.ws.get('items', 'haveSubSelector/' + encodeURI(this.selector.namespace));
                    if (response.error) {
                        this.notif.error(this.messages.get('dsi', response.errorMessage));
                    } else {
            ...
            getSelectorSort : async function() {
                if(this.selector.namespace !== "") {
                    this.sorts = await this.ws.get("items", "getSelectorSorts/" + encodeURI(this.selector.namespace));
                }
            }
		}
	}
