<template>
	<div id="list">
		<h3 v-if="!numanimation">
		  {{ pmb.getMessage("animation", "list_registration_title") }}</h3>
		<div class="row">
            <span>
				...
				<div class="row">
					<div class="colonne2">
						...
						<select v-model="locSelected">
							...
							<option v-for="(loc, index) in localisationlist"
								:value="loc.idlocation">{{ loc.location_libelle }}</option>
						</select>
					</div>
				</div>
			</div>
		...
		<table>
				...
				<th>
					<div class="center">
						<i class="fa fa-plus-square" style='cursor: pointer'
							@click="checkAll"></i> &nbsp; <i class="fa fa-minus-square"
							style='cursor: pointer' @click="uncheckAll"></i>
					</div>
				</th>
			...
			<template v-if="filteredRegistration.length > 0">
				<template v-for="(registration, index) in filteredRegistration">
					<tr
						v-if="
							(registration.animation.id ==  animSelected || 0 == animSelected) &&
							(registration.numRegistrationStatus == statusSelected || 0 == statusSelected)
							"
						:key='index' style='cursor: pointer' @mouseover="hover = index"
						@mouseout="hover = -1" @click="view(registration.idRegistration)"
						:class="[ index % 2 == 0 ? 'odd' : 'even', index == hover ? 'surbrillance' : '' ]">
						<td @click.stop="" style="cursor: default">
							<div class="center">
								<input type="checkbox" v-model="registrationsSelected"
						...
						<!-- <td v-if="numanimation" class="center" :title="pmb.getMessage('animation', 'anim_validate_registration')" >
								<button v-if="!registration.validated" :disabled="!checkValidation(index)" @click="validateRegistration(registration.id, index)"><i class="fa fa-check"></i></button>
							</td> -->
					</tr>
				</template>
			</template>
		...
		<div class="row">
			<div class="left">
				<input @click="validatedRegistrationList" class="bouton"
					type="button"
	...
	export default {
		...
		methods: {
			...
			delRegistration : function() {
				...
				var resultat = window.confirm(msg);
			...
			validateRegistration : function(id, index) {
				...
				var resultat = window.confirm(this.pmb.getMessage('animation', 'anim_confirm_validate_registration'));
			...
			checkValidation : function(index) {
				if (!this.registrationlist[index].validated && this.registrationlist[index].animation.allQuotas.availableQuotas.global >= this.registrationlist[index].nbRegisteredPersons) {
					return true;
				}
				return false;
			},
