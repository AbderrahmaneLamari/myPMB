<template>
	<div id="animationView">
		<h3 class="section-sub-title">
			...
			{{ animation.name }} ({{ animation.event.startDate }} {{ pmb.getMessage("animation", "animation_au") }} {{ animation.event.endDate }})
		</h3>
		<table>
			<tr>
				<td>
					...
					<div class="row">
						...
						{{ animation.event.startDate }} <template v-if="'00:00' !== animation.event.startHour">{{ animation.event.startHour}}</template>  
						{{ pmb.getMessage("animation", "animation_au") }} {{ animation.event.endDate }} <template v-if="'00:00' !== animation.event.endHour">{{ animation.event.endHour}}</template>
					</div>
					...
					<div class="row" v-if="animation.globalQuota >= 0 && !this.animation.hasChildrens || animation.internetQuota >=0 && !this.animation.hasChildrens">
						<div class="parent">
							<b>{{ pmb.getMessage("animation", "list_animation_quota") }} :</b>
								...
								<template v-if="animation.allQuotas.animationQuotas.global || animation.allQuotas.animationQuotas.internet ">
					...
					<br /><!-- avant les boutons -->
					<!-- BOUTONS -->
					<div class="row">
						<div class="left">
							<input @click="edit" class="bouton" type="button" :value="pmb.getMessage('animation', 'animation_modif')"/>
							<template v-if="animation.hasChildrens || animation.event.dateExpired">
							</template>
							<template v-else-if="animation.allQuotas.animationQuotas.global == 0 || animation.allQuotas.availableQuotas.global > 0 || animation.allowWaitingList == 1">
