...
function explnum_update($f_explnum_id, $f_notice, $f_bulletin, $f_nom, $f_url, $retour, $conservervignette=0, $f_statut_chk=0) {
	global $msg,$scanned_image,$scanned_image_ext ;
	...
	if ($f_explnum_id) {
		...
		if (($scanned_image)||($userfile_name)) {
			//Avant tout, y-a-t-il une image exterieure ?
			if ($scanned_image) {
				//Si oui !
	...
	//Dans le cas d'une modification, on regarde si il y a eu un deplacement du stockage
	if ($f_explnum_id){	
...
function explnum_add_from_url($f_notice_id, $f_bulletin_id, $f_nom, $f_url, $overwrite=true, $source_id=0, $filename='', $f_path='', $f_statut=0) {
	global $base_path;
