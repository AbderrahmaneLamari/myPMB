			...
			function cms_".$this->type."_deleted(response){
				...
				if(result.status == 'ok'){
					...
					switch(result.status){
						...
						case 'need_approval':
							/**
							 * Renvoyer les éléments nécessaire à la reconstruction de la requete dans la réponse (type + id)
					         */
							if(confirm(result.error_message)){
								var post = new http_request();
								post.request('./ajax.php?module=cms&categ=delete_".$this->type."',true,'&id='+document.forms['$name'].cms_editorial_form_obj_id.value+'&force_delete=1',true,cms_".$this->type."_deleted);
							}
							break;
					}
				}
		...
		if ($champ_trouve) {
				...
				foreach ($temp_callable as $k => $callable_data) {
					for ($i = 0; $i < count($callable_data); $i++) {
						...
						for($j=0 ; $j<count($callback_return) ; $j++) {
						    if (is_array($callback_return[$j])) {
						        foreach ($callback_return[$j] as $callback_lang => $callback_value) {
			...
			if(count($tab_pp)){
				foreach ( $tab_pp as $code_champ => $table ) {
       				...
       				$data=$p_perso->get_fields_recherche_mot($this->id, $this->type);
      				$j=0;
