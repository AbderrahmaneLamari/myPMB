...
// +-------------------------------------------------+
// $Id: onto_contribution_datatype_linked_work_selector_ui.class.php,v 1.4.2.2 2021/08/16 12:04:28 qvarin Exp $
...
// 		$selector_url = $this->get_resource_selector_url($property->range[0]);
		...
		if (!empty($datas)) {
			...
			foreach($datas as $key=>$data){
				...
				$selector = static::get_selector($property->pmb_name, $id, $name, (isset($formated_value['relation_type_work']) ? $formated_value['relation_type_work'] : ""));
			...
			$selector = static::get_selector($property->pmb_name, $id, $name, "");
			$inside_row = str_replace(array(
			    '!!onto_row_content_linked_record_selector!!',
			    "!!onto_row_content_marclist_range!!",
			    "!!onto_current_element!!",
	...
	protected static function get_selector($pmb_name, $id, $name, $selected) {
	    global $charset,$msg;
