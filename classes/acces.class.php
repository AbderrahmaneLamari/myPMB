...
// +-------------------------------------------------+
// $Id: acces.class.php,v 1.51.2.3 2023/11/20 16:28:45 dbellamy Exp $
            ...
            foreach ($this->dom['user']['property_link']['r_query'] as $k_var => $v_var) {
                switch ($v_var['type']) {
                    ...
                    case 'sql':
                        ...
                        break;
                    case 'callable':
                        $t_var[$k_var] = call_user_func_array(array($v_var["class"], $v_var["method"]), []);
                        break;
                    default:
                        break;
                }
...
class accesParser
{
    ...
    public function run($file)
    {
        ...
        if (!xml_parse($this->parser, $xml, TRUE)) {
            die(sprintf("erreur XML %s ï¿½ la ligne: %d", xml_error_string(xml_get_error_code($this->parser)), xml_get_current_line_number($this->parser)));
        }
