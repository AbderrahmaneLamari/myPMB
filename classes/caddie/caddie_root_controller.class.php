	...
	public static function get_display_list_from_item($type='display', $object_type='', $item=0) {
		...
		if(sizeof($liste)) {
			...
			foreach($print_cart["classement_list"] as $key => $cart_type) {
			    $display.=gen_plus(strtolower($object_type)."_".$key."_".$item,$cart_type["titre"],"<table border='0' cellspacing='0' style='width:100%' class='classementGen_tableau'>".$cart_type["cart_list"]."</table>",0);
			}
