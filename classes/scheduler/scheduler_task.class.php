...
// +-------------------------------------------------+
// $Id: scheduler_task.class.php,v 1.14.4.4 2023/09/12 07:55:46 dgoron Exp $
...
//	 				}
    						...
    						foreach($state["nextState"] as $nextState) {
    							...
    							foreach($commands as $command) {
    								if ($nextState["command"] == $command["name"]) {
    									if ($command["id"] == $cmd) {
    										$this->commande = $cmd;
    										$this->next_state = constant($nextState["value"]);
    										pmb_mysql_query("update taches set commande=".$this->commande.", next_state='".$this->next_state."' where id_tache=".$this->id_tache);
    										
    										scheduler_log::add_content('scheduler_'.scheduler_tasks::get_catalog_element($this->num_type_tache, 'NAME').'_task_'.$this->id_tache.'.log', '['.date('Y-m-d H:i:s').'] '.$msg['planificateur_command_'.$this->commande]);
    									}
    								}
    							}
    						}
    			...
    			//une commande de reprise sur une tâche de réindexation est-elle demandée ? 
    			if($type->get_name() == 'clean' && $this->is_more_recently()) {
    				if($cmd == RESUME) {
    					$this->commande = $cmd;
    					$this->next_state = RUNNING;
    					pmb_mysql_query("update taches set commande=".$this->commande.", next_state='".$this->next_state."' where id_tache=".$this->id_tache);
	...
	public function execute() {
		...
		if($this->statut == STOPPED && $this->commande == RESUME) {
	...
	public function recovery() {
		...
		//TODO : vérifier qu'une tâche n'est pas en cours
	}
	...
	public function run() {
		...
		$path_file = $base_path.'/admin/planificateur/run_task.php '.$this->id_tache.' '.$this->num_type_tache.' '.$this->num_planificateur.' '.$this->num_user.' '.$this->connectors_out_source_id.' '.LOCATION;
		$output=array();
	...
	public function finalize() {
		...
		if (round($this->indicat_progress) == 100) {
			...
			if(in_array($this->statut, array(SUSPENDED, STOPPED))) {
	...
	public function update_progression($percent) {
		...
		$query = "update taches set indicat_progress ='".$this->indicat_progress."', msg_statut = '".addslashes(encoding_normalize::json_encode($this->msg_statut))."', rapport='".htmlspecialchars(serialize($this->report), ENT_QUOTES,$charset)."' where id_tache=".$this->id_tache;
		pmb_mysql_query($query);
	}
...
//			}
			...
			$this->report[] = "<tr><td>".$msg["planificateur_write_success"]." : <a target='_blank' href='./tache_docnum.php?tache_docnum_id=".$scheduler_task_docnum->id."'>".$scheduler_task_docnum->nomfichier.".".$scheduler_task_docnum->extfichier."</a></td></tr>";
			return true;
		} else {
	...
	public function suspend() {
		...
		while ($this->statut == SUSPENDED && $this->suspended_time <= 600) {
			sleep(20);
	...
	protected function is_more_recently() {
		$query = "SELECT id_tache FROM taches WHERE num_planificateur = ".$this->num_planificateur." AND status=".STOPPED." ORDER BY id_tache DESC LIMIT 1";
		$result = pmb_mysql_query($query);
