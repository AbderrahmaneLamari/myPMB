...
// +-------------------------------------------------+
// $Id: z3950_notice.class.php,v 1.232.2.8 2022/01/10 16:39:34 qvarin Exp $
...
class z3950_notice {
	 ...
	 * @var array
	 */
	protected static $record_tmp = array();
	 ...
	 * @var array[]
	 */
	protected static $issue_tmp = array();
	...
	public function insert_in_database ($addslashes = false) {
		...
		if (empty($this->titles[0])) {
		    ...
		    pmb_mysql_query("INSERT INTO error_log (error_origin, error_text) VALUES ('import_" . addslashes(SESSid) . ".inc', '".$msg['277']."') ");
		    return;
		}
		...
		pmb_mysql_query("update notices set signature='$val' where notice_id=".$notice_retour);
...
// 			$nb_expl_ignores=0;
	...
	public function get_form ($action, $id_notice=0,$retour='link',$article=false) {
		...
		if($thesaurus_concepts_active == 1 && function_exists('traite_concepts_for_form')){
			...
			$traitement_rameau=traite_categories_for_form($tableau_600,$tableau_601,$tableau_602,$tableau_605,array(),$tableau_607,$tableau_608);
		} else {
		...
		if($thesaurus_concepts_active == 1 && function_exists('traite_concepts_for_form')){
			//tableau_606 traduit en concepts
			$manual_categorisation_form = get_manual_categorisation_form($tableau_600,$tableau_601,$tableau_602,$tableau_604,$tableau_605,array(),$tableau_607,$tableau_608);
		} else {
...
// 				$alink = '<a target="_blank" href="'.htmlspecialchars($doc_num["a"]).'">'.htmlspecialchars($doc_num["a"]).'</a>';
	...
	public function from_unimarc ($record) {
		...
		for ($i=0;$i<count($record->inner_directory);$i++) {
			...
			switch($cle) {
				...
				case "676": /* Dewey */
				    $this->dewey = $record->get_subfield($cle, "a");
				    $this->dewey["new_comment"] = $record->get_subfield($cle, "l");
					break;
				case "686": /* PCDM */
					if (!$this->dewey)
						$this->dewey=$record->get_subfield($cle,"a");
    ...
    public function import_notice_links() {
        ...
        if ($this->bibliographic_level == 'a' && $this->hierarchic_level == '2' && $tit_200d == 'Article_expl_bulletin') {
            // Quand on est sur le cas de la reconstruction d'un exemplaire de bulletin
            $issues = $this->get_records_linked_infos($notice, '463', 'bull_expl');
            $titles = [];
            foreach ($issues as $relation) {
                foreach ($relation as $field) {
                    switch ($field['label']) {
                        case '0':
                            $field0 = $field['content'];
                            break;
                        case 't':
                            $titles[] = $field['content'];
                            break;
                        case 'd':
                            $date = $field['content'];
                            break;
                        case 'e':
                            $mention = $field['content'];
                            break;
                        case 'v':
                            $num = $field['content'];
                            break;
            ...
            $id_serial = $this->create_record_serial($field0, $titles[1], $code);
            $issue = [
                'titre' => $titles[0],
                'date' => $date,
                'mention' => $mention,
        ...
        switch ($niveau_biblio) {
            case 's1':
                ...
                $articles = $this->get_records_linked_infos($notice, '464', 'art');
                $issues = $this->get_records_linked_infos($notice, '462', 'bull');
                $this->create_issues_and_articles($issues, $articles, $notice);
                break;
            case 'b2':
                // Lien vers une notice de bulletin
                $serials = $this->get_records_linked_infos($notice,'461','parent','b');
                $this->create_links_issues_notice($serials, $notice);
                break;
            case 'a2':
                // Lien vers un article
                $serials = $this->get_records_linked_infos($notice, '461', 'perio');
                $issues = $this->get_records_linked_infos($notice, '463', 'bull');
                $field = [
                    'id_base' => $id_unimarc,
                    'titre' => $this->titles[0]
                ];
        ...
        $this->create_parent_records_link($notice);
        $this->create_children_records_link($notice);
        
        // Traitement des relations horizontales
        $this->create_pairs_records_link($notice);
                ...
                switch ($this->notices_a_creer[$id_unimarc][$i]['lnk']) {
                    ...
                    case 'perio':
                        ...
                        if (empty($this->notices_crees[$id_unimarc])) {
                            ...
                            // Calcul des droits d'accès s'ils sont activés
                            $this->calculate_record_acces_rights($id_serial);
        ...
        if (!empty($this->bulletins_a_creer[$id_unimarc])) {
            ...
            for ($i = 0; $i < $nb_issues; $i++) {
                $issue = [
                    'titre' => $this->bulletins_a_creer[$id_unimarc][$i]['titre'],
                    'date' => $this->bulletins_a_creer[$id_unimarc][$i]['date_date'],
                    'mention' => $this->bulletins_a_creer[$id_unimarc][$i]['mention'],
                    'num' => $this->bulletins_a_creer[$id_unimarc][$i]['bull_num']
                ];
     ...
     * @return array
     */
    public function get_records_linked_infos($notice = [], $cle_unimarc = '', $lien = '', $type_lien = '') {
        $result_tab = [];
        if (!empty($cle_unimarc)) {
            $result_tab = $notice->get_subfield_array_array($cle_unimarc);
            if (!empty($lien) && !empty($result_tab)) {
                $options = $link_tab = [];
                foreach ($result_tab as $relation) {
                    foreach ($relation as $field) {
                        if ($field['label'] == 9) {
                            $parsed_string = explode(':', $field['content']);
                            if ($parsed_string[0] == 'lnk') {
                                $options['lien'] = $parsed_string[1];
                            }
                        }
                ...
                if (!empty($type_lien)) {
                    ...
                    foreach ($result_tab as $relation) {
                        foreach ($relation as $field) {
                            ...
                            if ($field['label'] == 9) {
                                $parsed_string = explode(':', $field['content']);
                                if ($parsed_string[0] == 'type_lnk') {
                                    $options['type_lnk'] = $parsed_string[1];
                                }
                            }
                            if (isset($options['type_lnk']) && $options['type_lnk'] == $type_lien) {
                                $type_link_tab[] = $relation;
                                break;
                            }
                        }
                    }
    ...
    public function create_issues_and_articles($tab_issues, $tab_art, $notice) {
        ...
        if (!empty($tab_issues)) {
            foreach ($tab_issues as $relation) {
                ...
                $issue = ['titre' => $titre, 'date' => $date, 'mention' => $mention, 'num' => $num];
                $this->create_issue($this->notice_id, $issue, '', '');
            }
        }
        ...
        if (!empty($tab_art)) {
            foreach ($tab_art as $relation) {
                $first_title = true;
                $id = $type_lnk = $lnk = $rank = $page = $tit_art = $tit = $date = $mention = $num = $field0 = "";
                foreach ($relation as $field) {
                    switch ($field['label']) {
                        case '9':
                            ...
                            switch ($splitted_string[0]) {
                                case 'id':
                                    $id = $splitted_string[1];
                                    break;
                                case 'type_lnk':
                                    $type_lnk = $splitted_string[1];
                                    break;
                                case 'lnk':
                                    $lnk = $splitted_string[1];
                                    break;
                                case 'rank':
                                    $rank = $splitted_string[1];
                                    break;
                                case 'page':
                                    $page = $splitted_string[1];
                                    break;
                            }
                            break;
                        case 't':
                            if ($first_title) {
                                $tit_art = $field['content'];
                                $first_title = false;
                            } else {
                                $tit = $field['content'];
                            }
                ...
                if (!isset($this->notices_crees[$id]) && empty($field0)) {
                    $issue = [
                        'titre' => $tit,
                        'date' => $date,
                        'mention' => $mention,
                    ...
                    if (!empty(static::$record_tmp) && !empty(static::$record_tmp[$id_unimarc_art])) {
                        ...
                        if (!empty($this->bulletins_crees) && !empty($this->bulletins_crees[$this->notice_id][$num]) && !empty($this->bulletins_crees[$this->notice_id][$num][$date.$mention])) {
                            // On supprimer le bulletin temp
                            if (!empty(static::$issue_tmp) && !empty(static::$issue_tmp[$id_unimarc_art])) {                                
                                $this->remove_issue(static::$issue_tmp[$id_unimarc_art]); 
                            }
                            $id_issue = $this->bulletins_crees[$this->notice_id][$num][$date.$mention];
                            $this->create_link_article_to_issue(static::$record_tmp[$id_unimarc_art], $id_issue);
                        }
                    } else {                        
                        // creation de l'article
                        $this->create_record_article($id_unimarc_art, $tit_art, $page, 0, $issue, "", "", $this->notice_id);
                    }
                } elseif (!empty($field0)) {
     ...
     * @param iso2709_record $notice
     */
    public function create_links_issues_notice($serials = [], $notice) {
        $id_unimarc = $notice->get_subfield('001');
        $id_unimarc = intval($id_unimarc[0]);
        
        $titre530 = trim($notice->get_subfield_array('530', 'a')[0]);
        ...
        if (empty($serials)) {
            if (!isset($this->notices_crees[$id_unimarc])) {
                ...
                $id_serial = $this->create_record_serial(0, $titre530, "");
                $issue = [
            ...
            foreach ($serials as $relation) {
                ...
                foreach ($relation as $field) {
                    switch ($field['label']) {
                        case '9':
                            $splitted_string = explode(':', $field['content']);
                            if ($splitted_string[0] == 'id') {
                                $id = intval($splitted_string[1]);
                            }
                            break;
                        case 'x':
                            $code = $field['content'];
                ...
                if (empty($this->notices_crees[$id]) && empty($field0)) {
                    // On créé le periodique car il n'est pas créé
                    if (empty($titre530)) {
                        $titre530 = 'Sans titre';
                    }
                    $id_serial = $this->create_record_serial($id, $titre530, $code);
                    $issue = [
                        'titre' => trim($notice->get_subfield_array('200', 'i')[0]),
                        'date' => trim($notice->get_subfield_array('210', 'h')[0]),
                        'mention' => trim($notice->get_subfield_array('210', 'd')[0]),
                    ...
                    $this->bulletins_a_creer[$field0] = [
                        'bull_notice' => $this->notices_crees[$id_unimarc],
                        'bull_num' => trim($notice->get_subfield_array('200', 'h')[0]),
                        'date_date' => trim($notice->get_subfield_array('210', 'h')[0]),
                        'mention' => trim($notice->get_subfield_array('210', 'd')[0]),
                        'titre' => trim($notice->get_subfield_array('200', 'i')[0])
                    ];
                } else {
    ...
    public function create_record_serial($old_id = 0, $tit1 = "", $code = 0) {
        ...
        if (!empty($old_id) && isset($this->notices_crees[$old_id])) {
            ...
            if (empty($id_serial) || $id_serial === false) {
                $query = "INSERT INTO notices(tit1, code, niveau_biblio, niveau_hierar, statut) VALUES('" . addslashes(clean_string($tit1)) . "', '" . addslashes($code) . "', 's', '1', '$this->statut')";
                pmb_mysql_query($query);
                $id_serial = pmb_mysql_insert_id();
                audit::insert_creation(AUDIT_NOTICE, $id_serial);
                // Mise à jour des index de la notice
                notice::majNotices($id_serial);
                // Mise à jour de la table "notices_global_index"
                notice::majNoticesGlobalIndex($id_serial);
                // Mise à jour de la table "notices_mots_global_index"
                notice::majNoticesMotsGlobalIndex($id_serial);
                if (!empty($old_id)) {
    ...
    public function create_issue($id_serial = 0, $issue_data = [], $title_serial = "", $code_serial = 0) {
        ...
        // On a rien, on créer le bulletin
        $query = "INSERT INTO bulletins(bulletin_numero, bulletin_notice, mention_date, date_date, bulletin_titre)
                    VALUES('" . addslashes($issue_data['num']) . "', '" . addslashes($id_serial) . "', '" . addslashes($issue_data['mention']) . "', '" . addslashes($issue_data['date']) . "', '" . addslashes($issue_data['titre']) . "')";
        pmb_mysql_query($query);
        $id_issue = pmb_mysql_insert_id();
        audit::insert_creation(AUDIT_BULLETIN, $id_issue);
    ...
    public function create_link_record_issue($old_id = 0, $id_serial = 0, $id_issue = 0, $id_rec_issue = 0, $title_rec_issue = "", $issue = []) {
        ...
        $query = "SELECT num_notice FROM bulletins WHERE bulletin_id = '$id_issue'";
        if (!empty($id_rec_issue)) {
            $query .= " AND num_notice != '$id_rec_issue'";
        }
        ...
        if (pmb_mysql_num_rows($res) && pmb_mysql_result($res, 0, 0)) {
            ...
            if (!empty($id_rec_issue)) {
                ...
                notice::del_notice($id_rec_issue);
                pmb_mysql_query("INSERT INTO error_log(error_origin, error_text) VALUES('import_" . addslashes(SESSid) . ".inc', '" . $msg[542] . addslashes(clean_string(implode(" ; ", $this->titles))) . "')");
            }
            $id_record_issue = pmb_mysql_result($res, 0, 0);
        } else {
            
            if (!empty($id_rec_issue)) {
                $id_record_issue = $id_rec_issue;
            } else {
                ...
                $query = "INSERT INTO notices(tit1, niveau_biblio, niveau_hierar, statut) VALUES('" . addslashes(clean_string($title_rec_issue)) . "', 'b', '2', '$this->statut')";
                pmb_mysql_query($query);
                $id_record_issue = pmb_mysql_insert_id();
                
                audit::insert_creation(AUDIT_NOTICE, $id_record_issue);
                // Calcul des droits d'accès s'ils sont activés
                if (function_exists("calc_notice_acces_rights")) {
                    calc_notice_acces_rights($id_record_issue);
                }
                // Mise à jour des index de la notice
                notice::majNotices($id_record_issue);
                // Mise à jour de la table "notices_global_index"
                notice::majNoticesGlobalIndex($id_record_issue);
                // Mise à jour de la table "notices_mots_global_index"
                notice::majNoticesMotsGlobalIndex($id_record_issue);
            }
            
            // On créer le lien entre le bulletin et la notice de bulletin
            $query = "UPDATE bulletins SET num_notice = '$id_record_issue' WHERE bulletin_id = '$id_issue'";
            pmb_mysql_query($query);
        }
        $this->notices_crees[$old_id] = $id_record_issue;
        
        //Lien entre la notice de bulletin et la notice de periodique
        notice_relations::insert($id_record_issue, $id_issue, 'b', 'up', false);
    ...
    public function create_links_articles($issues, $serials, $field, $notice) {
        ...
        if (empty($issues) && empty($serials)) {
            ...
            $serie_t = trim($notice->get_subfield_array('461', 't')[0]);
            $serie_v = trim($notice->get_subfield_array('461', 'v')[0]);
            if (!empty($serie_t) && !empty($serie_v)) {
                // Creation d'un périodique sans titre
                $id_serial = $this->create_record_serial(0, $serie_t, '');
                // J'ai les informations pour recréer le bulletinage
                $mention  = "";
                if (!empty($notice->get_subfield_array('210', 'd'))) {
                    $mention  = $notice->get_subfield_array('210', 'd')[0];                    
                }
                $id_issue = $this->create_issue($id_serial, [
                    'titre' => 'Bulletin N°' . $serie_v,
                    'date' => '',
                ...
                if (empty($field['id_base']) || (!empty($this->notices_crees[$field['id_base']]) && empty($this->notices_a_creer[$field['id_base']]))) {
                    // On regarde si elle n'est pas reliée a un bulletin
                    $query = "SELECT analysis_bulletin FROM analysis WHERE analysis_notice='" . addslashes($this->notices_crees[$field['id_base']]) . "'";
                    $res = pmb_mysql_query($query);
                    if (!pmb_mysql_num_rows($res)) {
                ...
                } elseif (empty($this->notices_crees[$field['id_base']]) && empty($this->notices_a_creer[$field['id_base']])) {
                    if ($this->notice_id) {
                        ...
                        $query = "INSERT INTO notices(tit1, niveau_biblio, niveau_hierar) VALUES('" . addslashes($field['titre']) . "', 'm', '0')";
                        pmb_mysql_query($query);
                        $this->notices_crees[$field['id_base']] = pmb_mysql_insert_id();
                        audit::insert_creation(AUDIT_NOTICE, $this->notices_crees[$field['id_base']]);
                        // Calcul des droits d'accès s'ils sont activés
                        $this->calculate_record_acces_rights($this->notices_crees[$field['id_base']]);
                    }
                }
            }
            ...
            foreach ($issues as $relation) {
                $title = $num = $date = $mention = '';
                foreach ($relation as $relation_field) {
                    switch ($relation_field['label']) {
                        case 't':
                            $title = $relation_field['content'];
                            break;
                        case 'v':
                            $num = $relation_field['content'];
                            break;
                        case 'd':
                            $date = $relation_field['content'];
                            break;
                        case 'e':
                            $mention = $relation_field['content'];
                            break;
                    }
                }
            ...
            foreach ($serials as $relation) {
                $id = $type_lnk = $lnk = $rank = $field0 = $title = $code = '';
                foreach ($relation as $relation_field) {
                    switch ($relation_field['label']) {
                        case '9':
                            $splitted_string = explode(':', $relation_field['content']);
                            switch ($splitted_string[0]) {
                                case 'id':
                                    $id = $splitted_string[1];
                                    break;
                            ...
                            break;
                        case '0':
                            $field0 = $relation_field['content'];
                            break;
                        case 't':
                            $title = $relation_field['content'];
                            break;
                        case 'x':
                            $code = $relation_field['content'];
                            break;
                    }
                }
                ...
                if (empty($this->notices_crees[$id]) && empty($field0)) {
                    ...
                    $id_serial = $this->create_record_serial($id, $title, $code);
                    $id_issue = $this->create_issue($id_serial);
                    $this->create_link_article_to_issue($this->notice_id, $id_issue);
                } elseif (!empty($this->notices_crees[$id])) {
            ...
            for ($i = 0; $i < $nb_serials; $i++) {
                $id = $type_lnk = $lnk = $rank = $field0 = $serial_title = $code = '';
                foreach ($serials[$i] as $relation_field) {
                    switch ($relation_field['label']) {
                        case '9':
                            $splitted_string = explode(':', $relation_field['content']);
                            switch ($splitted_string[0]) {
                        ...
                        case '0':
                            ...
                            break;
                        case 't':
                            $serial_title = $relation_field['content'];
                            break;
                        case 'x':
                            $code = $relation_field['content'];
                            break;
                ...
                $issue_title = $date = $mention = $num = '';
                foreach ($issues[$i] as $relation_field) {
                    switch ($relation_field['label']) {
                        case 't':
                            $issue_title = $relation_field['content'];
                            break;
                        case 'd':
                            $date = $relation_field['content'];
                            break;
                ...
                if (empty($this->notices_crees[$id]) && empty($field0)) {
                    ...
                    $id_serial = $this->create_record_serial($id, $serial_title, $code);
                    $id_issue = $this->create_issue($id_serial, [
                        'titre' => $issue_title,
                        'date' => $date,
                        'mention' => $mention,
                    ...
                    $this->notices_a_creer[$id][] = [
                        ...
                        'lnk' => $lnk,
                        'rank' => $rank,
                        'titre_perio' => $serial_title,
                        'code' => $code,
                        'id_asso' => $this->notice_id,
                        'num' => $num,
                        'mention' => $mention,
                        'date' => $date,
                        'titre_bull' => $issue_title
                    ];
                }
            }
        }
    ...
    /**
     * Création des relations entre notices
     * 
     * @param array $records
     */
    public function create_links_records($records = []) {
        foreach ($records as $relation) {
            $titles = [];
            $id = $type_lnk = $lnk = $rank = "";
            foreach ($relation as $field) {
                switch ($field['label']) {
                    case '9':
                        $splitted_string = explode(':', $field['content']);
                        switch ($splitted_string[0]) {
                            ...
                            case 'rank':
                                ...
                                break;
                            case 'bl':
                                $bibliographic_level = $splitted_string[1];
                                break;
                        }
                        break;
                    case 't':
                    ...
                    case '0':
                        ...
                        break;
                    case 'y':
                        $code_y = $field['content'];
                        break;
                    case 'x':
                        $code_x = $field['content'];
                        break;
                }
            }
            ...
            if (!empty($this->notices_crees[$id])) {
                ...
                switch ($bibliographic_level) {
                    case 'm0':
                        $id_record_linked = $this->create_record_monograph($id, $titles[0], $code_y);
                        break;
                    case 's1':
                        $id_record_linked = $this->create_record_serial($id, $titles[0], $code_x);
                        break;
                    case 'a2':
                        $issue = [
                            'titre' => $titles[1],
                        ...
                        $id_record_linked = $this->create_record_article($id, $titles[0], '', '', $issue, $titles[2], $code_x, '');
                        break;
                    case 'b2':
                        $issue = [
                            'titre' => $titles[1],
                        ...
                        $id_record_linked = $this->create_record_issue($id, $titles[0], $issue, $titles[2], $code_x);
                        break;
                }
            }
            ...
            if (!empty($this->notice_id) && !empty($id_record_linked) && ($this->notice_id != $id_record_linked)) {
                if (! notice_relations::relation_exists($this->notice_id, $id_record_linked, $type_lnk, $direction)) {
                    notice_relations::insert_from_import($this->notice_id, $id_record_linked, $type_lnk, $rank, $direction);
                }
            }
        }
    }
     ...
     */
    public function create_record_monograph($old_id = 0, $title = "", $code = "") {
        ...
        if (isISBN($code)) {
            $query = "SELECT notice_id FROM notices WHERE niveau_biblio = 'm' AND niveau_hierar = '0' AND code = '" . addslashes($code) . "'";
            $res = pmb_mysql_query($query);
            if (pmb_mysql_num_rows($res)) {
    ...
    /**
     * Calcul des droits d'accès à la notice
     * 
     * @param int|string $id_record
     */
    public function calculate_record_acces_rights($id_record) {
        global $gestion_acces_active, $gestion_acces_user_notice, $gestion_acces_empr_notice;
        ...
        if ($gestion_acces_active == 1) {
            ...
            if ($gestion_acces_user_notice == 1) {
                $dom_1 = $ac->setDomain(1);
                $dom_1->storeUserRights(0, $id_record);
            }
            // Traitement des droits acces empr_notice
            if ($gestion_acces_empr_notice == 1) {
                $dom_2 = $ac->setDomain(2);
                $dom_2->storeUserRights(0, $id_record);
            }
        }
    }
    ...
    public function create_link_article_to_issue($id_article, $id_issue) 
    {
        ...
        $query = "SELECT notice_id FROM notices WHERE notice_id = '" . addslashes($id_article) . "'";
        $res = pmb_mysql_query($query);
        if (pmb_mysql_num_rows($res)) {
            // On verifie que le lien entre le bulletin et l'article n'est pas deja present
            $query = "SELECT 1 FROM analysis WHERE analysis_notice = '" . addslashes($id_article) . "' AND analysis_bulletin = '" . addslashes($id_issue) . "'";
            $res = pmb_mysql_query($query);
            if (!pmb_mysql_num_rows($res)) {                
                // On créer le lien entre le bulletin et l'article
                $query = "INSERT INTO analysis(analysis_bulletin, analysis_notice) VALUES('" . addslashes($id_issue) . "', '" . addslashes($id_article) . "' )";
                pmb_mysql_query($query);
                return true;
            }
        }
     ...
     * @return boolean
     */
    public function remove_issue($id_issue) 
    {
        $query = "SELECT 1 FROM bulletins WHERE bulletin_id = '" . addslashes($id_issue) . "'";
        $result = pmb_mysql_query($query);
        if (pmb_mysql_num_rows($result)) {
            $id_issue = intval($id_issue);
            $bulletinage = new bulletinage($id_issue);
            $bulletinage->delete();
            return true;
        }
        return false;
...
//         }
     ...
     * @param iso2709_record $notice
     */
    private function create_parent_records_link($notice) {
        $parents = $this->get_records_linked_infos($notice, '', 'parent');
        if (!empty($parents)) {
            ...
            if (!empty($parents)) {
                $this->create_links_records($parents);
            }
        }
    }
    
    /**
     * Création des relations enfantes
     * @param iso2709_record $notice
     */
    private function create_children_records_link($notice) {
        $children = $this->get_records_linked_infos($notice, '', 'child');
        if (!empty($children)) {
            foreach ($children as $key => $relation) {
                $title = "";
                foreach ($relation as $field) {
                    if ($field['label'] == 't') {
                        $title = $field['content'];
            ...
            // Reste-t-il des relations à créer après le nettoyage ci-dessus ?
            if (!empty($children)) {
                $this->create_links_records($children);
            }
        }
    }
    
    /**
     * Création des relations horizontales
     * @param iso2709_record $notice
     */
    private function create_pairs_records_link($notice) {
        $pairs = $this->get_records_linked_infos($notice, '', 'pair');
        if (!empty($pairs)) {
            foreach ($pairs as $key => $relation) {
                $title = "";
                foreach ($relation as $field) {
                    if ($field['label'] == 't') {
                        $title = $field['content'];
            ...
            // Reste-t-il des relations à créer après le nettoyage ci-dessus ?
            if (!empty($pairs)) {
                $this->create_links_records($pairs);
            }
        }
    }
}