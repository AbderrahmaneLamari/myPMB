...
// +-------------------------------------------------+
// $Id: indexation_aut_link.class.php,v 1.1.2.2 2023/05/13 16:41:46 dgoron Exp $
...
// 				'LANGUAGE' => array(),
		...
		if ($this->entity_type == $linked_authority_type) {
			$structure['ALIAS'] = $table.'_link';
			if($this->type == 'entities') {
				$structure['FILTER'] = array(
						array('value' => $table.'_link.'.$tablekey_entity.' != '.$table.'.'.$tablekey_entity)
						
				);
			} else {
				$structure['FILTER'] = array(
						array('value' => $table.'_link.'.$tablekey_entity.' != !!object_id!!')
