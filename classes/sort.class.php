...
global $ligne_tableau_tris, $ligne_tableau_tris_etagere, $ligne_tableau_tris_rss_flux, $ligne_tableau_external_tris, $popup_ligne_tableau_tris;
...
class sort {
	...
	private function get_template_tris($type = "NOTI", $popup = false) {
	    ...
	    global $ligne_tableau_tris_segment;
	    global $popup_ligne_tableau_tris_segment;
	    global $categ;
	        ...
	        switch ($this->caller) {
                ...
                default:
                    if($categ == "search_universes"){
                        $tristemp = str_replace("!!id_tri!!", $result['id_tri'], ($popup ? $popup_ligne_tableau_tris_segment : $ligne_tableau_tris_segment));
                        $tristemp = str_replace("!!segment_id!!", $this->dSort->posParcours, ($popup ? $popup_ligne_tableau_tris_segment : $ligne_tableau_tris_segment)); 
                    } else {
	...
	public function show_sel_formAdmin($id_tri, $index_tri=null, $sort_string=null) {
		...
		if (isset($num_segment)) {
		    $sel_form = str_replace("!!id_segment!!", $num_segment, $show_sel_form_segments);
		}
