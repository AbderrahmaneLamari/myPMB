...
// +-------------------------------------------------+
// $Id: interface_entity_reader_form.class.php,v 1.5.2.2 2022/01/25 08:14:19 gneveu Exp $
...
class interface_entity_reader_form extends interface_entity_form {
	    ...
	    $html .= "
            <script type='text/javascript'>
    			function empr_form_submit(node) {
                    ...
                    let empr_password_mail_disabled = document.getElementById('form_empr_password_mail').disabled;
	...
	protected function get_locations_selector() {
		...
		if ($pmb_lecteurs_localises) {
			if($pmb_form_empr_editables==2) {
				return docs_location::get_html_select(array(0),array('id'=>0,'msg'=>$msg['all_locations_empr']),array('id'=>'empr_grille_location','class'=>'saisie-20em','style'=>'display:none;', 'backbone'=>'yes'));
			} else {
				return docs_location::get_html_select(array(0),array('id'=>0,'msg'=>$msg['all_locations_empr']),array('id'=>'empr_grille_location','class'=>'saisie-20em','style'=>'display:none;', 'onChange'=>'get_pos(); expandAll(); if (inedit) move_parse_dom(relative); else initIt();'));
			}
		} else {
